...
- `fx-script`: string[11] parallel to the Gauntlet STEPS — Corbin's editable cold-call script (Playbook). `fx-objections`: `{trigger,response}[]` — his editable objection/stall wall. Both seeded from DEFAULT_SCRIPT / DEFAULT_OBJECTIONS, edited in-app (no deploy needed), live in the Hunt tab behind a `[Tallies | Script | Objections]` toggle (`huntView`).
