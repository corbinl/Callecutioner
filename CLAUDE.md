...
The daily loop: **Dawn** (5-min ritual before first dial) → **Hunt** (live tap tallies during the block) → **Card** (90-sec report card after real conversations) → **Forge** (v1.3.0: Dennehy methodology mastery — library / spaced-rep drills / sparring / homework quests / mastery progress) → **Dusk** (5-min close, banks XP, extends streak) → **Ledger** (analytics).

**Six bottom tabs as of v1.3.0** (the old "tabs stay 5" rule was amended 2026-07-11 with Corbin's sign-off when the Forge shipped as a standalone everyday area).

**Mastery XP track (v1.3.0):** `fx-mastery` holds a SEPARATE XP pool (`⚔`, MASTERY_RANKS ladder) for Forge behaviours only: drill reps, spar reps, step-order reconstruction, homework quests, full Card dissections, One Things internalized. It must never mix with call XP: do not add terms to `dayXP()` and do not touch `fx-meta`. **Anti-avoidance governor:** mastery XP pays half rate on days with zero dials (`masteryAward()`), so studying never outranks dialing. That governor is as load-bearing as the XP-for-behaviours rule.
...
- `content.js` — The Forge corpus (v1.3.0+): auto-assembled from the VIP Coaching Club docs, exposes one global `FORGE_CONTENT`, precached by sw.js, loaded before the main script. Do NOT hand-edit; rebuild from the extraction pipeline (see docs/forge-design.md).
...
**Also embedded in Legendary Routes** (2026-07-11): LR vendors these files into `client/public/apps/callexecutioner/` and serves them at `/apps/callexecutioner/` behind a platinum sidebar tab ("Callexecutioner", feature-flagged). After every release here, run `npm run sync:callex` in the LR repo. LR is home base; GitHub Pages is the offline fallback. Sealed integration — no data exchange with the CRM (see LR's `acquisitions/callexecutioner/DEAL-MEMO.md`).
...
- `fx-carddraft`: in-progress card. `fx-tab`: last open tab. `fx-forgeview`: last Forge sub-view.
- `fx-mastery` (v1.3.0): `{xp, drillState:{cardId:{box,due}}, quests:{id:bool}, questsAwarded:{id:true}, sparDone, sparGot, oneThingLog:[{text,date}], pinDismissed:[text]}`. Leitner boxes 1–5, intervals 0/1/2/4/7/14 days. Quest XP pays once per id (`questsAwarded`), so untick/retick can't farm it.
- Card shape additions (v1.3.0): `mistakes:{talkedTooMuch,pitchedEarly,priceNoContext,pushedTooSoon,didntListen}` (Y/N/null, S13 rubric) and `weakFund` (fundamental KEY, e.g. "socratic" — routes the Drill deck). Legacy cards without them are fine (guards everywhere).
...
**Meeting anatomy (10 steps):** Omega → Mock presentation → Future state → Pain discovery questions → Five pointers → Selling the proposal → The 3.5 alternatives → Next steps → Pulling the pin → Ending the meeting. ⚠️ ORDER IS A RECONSTRUCTION from notebook fragments, not verified against Dennehy's teaching. Corbin will confirm/correct after a Coaching Club session. When he does, update MEETING_STEPS. **Transcript sweep 2026-07-11: all 23 VIP Coaching Club transcripts searched — Dennehy never enumerates a numbered meeting anatomy (or an "11-step gauntlet", or "five pointers" by name). Both stay flagged for Ben.** What IS verified from transcripts: up-front contract comes first and its wording, the openers, the permission ask (all now in FORGE_CONTENT.quickref, verbatim-tagged).

**Five pointers (meeting qualification gates):** Problems we fix / They accept it's a problem / They convinced ME (direction matters: prospect convinces the seller) / Can afford it / How they decide. ⚠️ The phrase "five pointers" appears nowhere in the transcripts. Ben's verified five-list is the **five buyer motivators**: pain in the present (strongest) → pain in the future → pleasure in the present → pleasure in the future → curiosity. Ask Ben whether/how these map.

**Answer Key caveat:** the Mastery Worksheets Answer Key is AI-generated and Corbin hasn't reviewed it. 17/22 recall answers were grounded against the session docs; the other 5 (S01, S09, S10, S17, S18) render with an "AI-drafted — verify" badge in the Drill deck until confirmed.
